...
int right_front_spd = 255;  //左邊前輪PWM的速度（0~255）  左邊速度要比右邊少30
int right_back_spd = 250;  //左邊後輪PWM的速度（0~255）
int left_front_spd = 255;  //右邊前輪PWM的速度
int left_back_spd = 250;   //右邊後輪PWM的速度
...
void moveForward()
{
  ...
  analogWrite(ENA1, right_front_spd);
  analogWrite(ENB1, right_back_spd);

  digitalWrite(IN5, LOW);
  digitalWrite(IN6, HIGH);
  digitalWrite(IN7, HIGH);
  digitalWrite(IN8, LOW);
  analogWrite(ENA2, left_back_spd);
  analogWrite(ENB2, left_front_spd);
}
